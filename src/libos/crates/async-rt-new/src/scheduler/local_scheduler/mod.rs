...
impl<E: SchedEntity> LocalScheduler<E> {
        ...
        S: StatusNotifier + 'static,
    {
        Self {
            this_vcpu,
            rqs: Mutex::new((
                [Box::new(RunQueues::new()), Box::new(RunQueues::new())],
                0)),
            len: AtomicU32::new(0),
            parker: Parker::new(),
    ...
    fn try_dequeue(&self) -> Option<Arc<E>> {
        loop {
            ...
            if let Some(entity) = front_rqs.try_dequeue() {
                ...
                debug_assert!(entity.sched_state().timeslice() > 0);
    ...
    ///
    /// TODO: rewrite the document
    ///
    /// Similar to the `dequeue` method, the `drain` method removes entities
    /// from the scheduler. The main difference is three points. First,
    /// the `drain` method removes 
    /// The maximum number of entities drained is equal to `Vec::capacity()`
    /// - `Vec::len()`. Entities with low effective priorities are drained first.
    /// The number of entities drained will be returned.
    ///
        ...
        F: FnMut(&E) -> bool,
    {
        ...
        let mut drain_queue = |rqs: &mut RunQueues<E>| {
            ...
            for q in rqs.run_queues.iter_mut().rev() {
                let mut i = 0;
                while i < q.len() {
                    let entity = &q[i];
                    if (cond)(entity) {
